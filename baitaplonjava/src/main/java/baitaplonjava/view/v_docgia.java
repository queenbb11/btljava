...
public class v_docgia extends JFrame {
    ...
    private void initComponents() {
        ...
        int y = 70;
        addLabel("Mã ĐG:", 50, y);
        txtMaDG = addTextField(170, y);

        addLabel("Tên ĐG:", 50, y += 40);
        txtTenDG = addTextField(170, y);
        ...
        addLabel("Email:", 50, y += 40);
        txtEmail = addTextField(170, y);
        ...
        lbSearch.setBounds(50, 380, 80, 30);
        add(lbSearch);

        txtTimkiem = new JTextField();
        txtTimkiem.setBounds(130, 380, 300, 30);
        add(txtTimkiem);

        btnTimkiem = addButton("Tìm", 440, 380);
        btnXuatfile = addButton("Xuất file", 560, 380);
        btnBack = addButton("Quay lại", 700, 380);

        // ===== TABLE =====
        String[] cols = {
            "Mã ĐG", "Tên ĐG", "Ngày sinh",
    ...
    // ===== GET MODEL + VALIDATE =====
    public m_docgia getDocgia() {

        String ma = txtMaDG.getText().trim();
        String ten = txtTenDG.getText().trim();
